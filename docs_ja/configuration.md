...
<tr>
  ...
  <td>
    ...
    If off-heap memory use is enabled, then <code>spark.memory.offHeap.size</code> must be positive.
    <!-- /en --><!-- ja -->
    true の場合、 Spark は off-heap メモリを使おうと試みます。
    off-heap メモリの使用が有効になっている場合、
    <code>spark.memory.offHeap.size</code> は正の値でなければいけません。
    <!-- /ja -->
  </td>
</tr>
<tr>
...
<tr>
  ...
  <td>
    ...
    <!-- /en --><!-- ja -->
    YARN モードでは 1、スタンドアロンモードと Mesos の coarse-grained モードではワーカ上の利用できるすべてのコア。
    <!-- /ja -->
  </td>
  <td>
    <!-- en -->
    ...
    スタンドアロンモードと Mesos の coarse-grained モードでは、
    ワーカ上に十分なコアがある場合、
    このパラメータを設定することにより
...
<tr>
  ...
  <td>
    ...
    <!-- /en --><!-- ja -->
    <code>true</code> の場合、
    Spark は <code>StreamingContext</code> を即座にシャットダウンするのではなく、
    JVM 上でグレースフルにシャットダウンします。
    <!-- /ja -->
  </td>
</tr>
<tr>
...
# Inheriting Hadoop Cluster Configuration // Hadoop クラスタの設定を引き継ぐ

<!-- en -->
If you plan to read and write from HDFS using Spark,
there are two Hadoop configuration files that should be included on Spark's classpath:
<!-- /en --><!-- ja -->
Spark を使用した HDFS の読み書きを計画している場合、
Spark のクラスパスに含まれているべき Hadoop の設定ファイルが 2 つあります。
<!-- /ja -->

* <!-- en -->
  `hdfs-site.xml`, which provides default behaviors for the HDFS client.
  <!-- /en --><!-- ja -->
  `hdfs-site.xml` … HDFS クライアントのデフォルトのふるまいを提供する
  <!-- /ja -->
* <!-- en -->
  `core-site.xml`, which sets the default filesystem name.
  <!-- /en --><!-- ja -->
  `core-site.xml` … デフォルトのファイルシステム名をセットする
  <!-- /ja -->

<!-- en -->
The location of these configuration files varies across CDH and HDP versions,
 but a common location is inside of `/etc/hadoop/conf`.
Some tools, such as Cloudera Manager,
 create configurations on-the-fly, but offer a mechanisms to download copies of them.
<!-- /en --><!-- ja -->
これらの設定ファイルの場所は CDH 版と HDP 版とで違いがありますが、
共通の場所は `/etc/hadoop/conf` の内側です。
Cloudera Manager のようないくつかのツールは設定をオンザフライで生成しますが、
それらのコピーをダウンロードする仕組みが提示されています。
<!-- /ja -->
...
<!-- /en --><!-- ja -->
これらのファイルを Spark から見えるようにするには、
`$SPARK_HOME/spark-env.sh` 内の `HADOOP_CONF_DIR` に該当の設定ファイルを含む場所をセットします。
<!-- /ja -->
